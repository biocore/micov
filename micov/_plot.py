...
def slice_positions(positions, id_):
    return (
        ...
        .select(
            pl.col(COLUMN_GENOME_ID), pl.col(COLUMN_START), pl.col(COLUMN_STOP)
        )
    )
...
def per_sample_plots(
    ...
    for genome in all_coverage[COLUMN_GENOME_ID].unique():
        non_cumulative(
            metadata, all_coverage, genome, sample_metadata_column, output
        )
        cumulative(
            metadata,
            all_coverage,
...
def cumulative(metadata, coverage, positions, target, variable, output):
    ...
    for (name,), grp in metadata.group_by(
        [
            variable,
        ],
        maintain_order=True,
    ...
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 100)
    ax.set_title((f"Cumulative: {target} " f"({length}bp){stat}"), fontsize=20)
...
def non_cumulative(metadata, coverage, target, variable, output):
    ...
    ax.set_ylim(0, 100)
    ax.set_title(
        (f"Non-cumulative: {target} " f"({length}bp){stat}"), fontsize=20
    )
    plt.legend(labels, fontsize=20)
...
def position_plot(
    metadata, coverage, positions, target, variable, output, scale=None
):
    plt.figure(figsize=(12, 8))
    ax = plt.gca()
    labels = []
    colors = []
    target_positions = positions.filter(
        pl.col(COLUMN_GENOME_ID) == target
    ).lazy()

    for ((name,), grp), color in zip(
        metadata.group_by(
            [
                variable,
            ],
        ...
        for sid, gid, x in grp_coverage[col_selection].rows():
            ...
            if scale is None:
                covered_positions = _get_covered(
                    cur_positions.collect().to_numpy()
                )
    ...
    if scale is None:
        ...
        ax.set_title(
            f"Scaled position plot: {target} ({length}bp)", fontsize=20
        )
