...
@click.option(
    ...
    type=click.Path(exists=True),
    required=True,
    help="Output directory. If new, will be created.",
)
@click.option(
    "-g",
    "--genome-id",
    type=str,
    required=True,
    help="Genome ID of the genome of interest",
)
@click.option(
    "-l",
    "--genome-length",
    type=int,
    required=True,
    help="Length of the genome of interest",
)
@click.option(
    "-n", "--bin-num", type=int, default=1000, required=False, help="Number of bins"
)
def binning(covered_positions, outdir, genome_id, genome_length, bin_num):
    ...
    pos_updated.write_csv(
        f"{outdir}/pos_binned.tsv", separator="\t", include_header=True
    )
