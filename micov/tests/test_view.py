...
from micov._constants import (COLUMN_GENOME_ID, COLUMN_SAMPLE_ID, COLUMN_START,
                              COLUMN_START_DTYPE, COLUMN_STOP_DTYPE, COLUMN_STOP,
                              COLUMN_LENGTH, COLUMN_COVERED_DTYPE, COLUMN_LENGTH_DTYPE,
                              COLUMN_PERCENT_COVERED, COLUMN_PERCENT_COVERED_DTYPE,
                              COLUMN_COVERED)


def make_cov_pos(d, name):
    (pl.LazyFrame([['G1', 'S1', 8, 100, 8.0],
                   ['G2', 'S1', 20, 100, 20.0],
                   ['G3', 'S1', 30, 100, 30.0],
                   ['G3', 'S2', 11, 100, 11.0],
                   ['G4', 'S2', 21, 100, 21.0],
...
class ViewTests(unittest.TestCase):
    ...
    def test_view_sample_superset(self):
        ...
        plt.assert_frame_equal(obs_md, exp_md, check_column_order=False, check_row_order=False)
        plt.assert_frame_equal(obs_cov, exp_cov, check_column_order=False, check_row_order=False)
        plt.assert_frame_equal(obs_pos, exp_pos, check_column_order=False, check_row_order=False)
    ...
    def test_view_constrain_positions_full(self):
        ...
        exp_cov = (pl.read_parquet(f"{self.d}/{self.name}.coverage.parquet")
                     .filter(pl.col(COLUMN_GENOME_ID).is_in(['G1', 'G5']))
                     .with_columns(pl.col(COLUMN_LENGTH) * 10,
                                   pl.col(COLUMN_PERCENT_COVERED) / 10))
        exp_pos = (pl.read_parquet(f"{self.d}/{self.name}.covered_positions.parquet")
                     .filter(pl.col(COLUMN_GENOME_ID).is_in(['G1', 'G5'])))
    ...
    def test_view_constrain_positions_none(self):
        feat = pl.DataFrame([['G1', 1000, 2000],
                             ['G5', 1000, 2000]],
                            orient='row',
                            schema=[(COLUMN_GENOME_ID, str),
                                    (COLUMN_START, COLUMN_START_DTYPE),
    ...
    def test_view_constrain_positions_bounds_simple(self):
        ...
        exp_cov = pl.DataFrame([['G1', 'S1', 2, 2, 100.0],
                                ['G5', 'S3', 10, 20, 50.0]],
                               orient='row',
                               schema=[(COLUMN_GENOME_ID, str),
                                       (COLUMN_SAMPLE_ID, str),
        ...
        # start is retained
        exp_pos = pl.DataFrame([['G1', 'S1', 7, 9],
                                ['G5', 'S3', 10, 20]],
                               orient='row',
                               schema=[(COLUMN_GENOME_ID, str),
                                       (COLUMN_SAMPLE_ID, str),
    ...
    def test_view_constrain_positions_bounds_complex(self):
        ...
        exp_cov = pl.DataFrame([['G1', 'S1', 8, 100, 8.0],
                                ['G2', 'S1', 10, 20, 50.0],
                                ['G2', 'S3', 12, 20, 60.0],
                                ['G3', 'S3', 20, 20, 100.0],
                                ['G5', 'S3', 20, 20, 100.0]],
        ...
        exp_pos = pl.DataFrame([['G1', 'S1', 1, 5],
                                ['G1', 'S1', 6, 10],
                                ['G2', 'S1', 40, 50],
                                ['G2', 'S3', 40, 52],
                                ['G3', 'S3', 40, 60],
